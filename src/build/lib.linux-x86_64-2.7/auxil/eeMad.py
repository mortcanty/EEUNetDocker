...
def covarw(image, weights, scale=10, maxPixels=1e9):
...
#  normalize kth band in target    
    ...
    return ee.Dictionary({'image':image,'scale':scale,'ncmask':ncmask,'nbands':nbands,'rect':rect,'coeffs':coeffs,'normalized':normalized})    
...
def imad1(current,prev): 
    ...
    prev = ee.Dictionary(prev)
    image = ee.Image(prev.get('image'))
    chi2 = ee.Image(prev.get('chi2'))      
    allrhos = ee.List(prev.get('allrhos'))
    scale = ee.Number(prev.get('scale'))
...
#  chi square image
    chi2 = MAD.pow(2) \
              ...
              .clip(region)                
    return ee.Dictionary({'done':done,'scale':scale,'image':image,'allrhos':allrhos,'chi2':chi2,'MAD':MAD})     
